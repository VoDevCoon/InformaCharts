    ...
    wooCommerce.getAsync(query).then((result) => {
      if (result.statusCode === 200 && result.body.length > 0) {
        ...
        if (orders.length === pageSize) {
          const nextPage = pageIndex + 1;
          searchNewOrders(wooCommerce, event, createdDate, nextPage, pageSize).then(() => resolve());
        } else {
...
const seed = {
  newEvents: () => {
    Event.findOne({}).sort({ createdDate: -1 }).exec().then((result) => {
      ...
      searchNewEvents(wooCommerceAPI, createdDateString, 1, 100);
    });
  },
  ...
  newEventOrders: async (event) => {
    ...
    return new Promise((resolve) => {
      Order.findOne({ event: event._id }).sort({ createdDate: -1 }).exec().then((result) => {
        ...
        searchNewOrders(wooCommerceAPI, event, createdDateString, 1, 20).then(() => resolve());
      });
    });
  },
};
